...
public class findRight {
    //求<=num的最右位置
    //eg:2 4 5 5 5 6 8 9  求<=5的最右位置是4，求<=7的最左位置是6 找不到就返回-1
    //   0 1 2 3 4 5 6 7
    ...
    public static int validator(int[] arr, int num) {
        for (int i = 0; i < arr.length; i++) {
            //遍历找到第一个大于num的数
            if (arr[i] >num) {
                //如果找到了就返回i-1也就是数组中比num大的前一个数的坐标
                //极端条件：数组全部的数都比num ，那么比num大的第一个数就是下标为0，刚好0-1=-1满足找不到就返回
                return i-1;
            }
        }
        //极端条件：如果遍历完还没有找到说明整个数组都小于num，返回arr.length-1即可
        return arr.length-1;
    }
    ...
    public static int findLeft(int[] arr, int num) {
        ...
        while (leftIndex <= rightIndex) {
            ...
            if (arr[middleIndex] <= num) {
                //如果中点<=num,说明右侧侧一定有>=num的数，更新ans的坐标
                ans = middleIndex;
                //继续二分，在左侧继续寻找
